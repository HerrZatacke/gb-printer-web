...
function LocalDebug() {
  ...
  const { raw: rawImages1 } = useImages({ raw: { filters: { tags: ['testing'] }, sort: { field: 'created', direction: 'asc' }, page: 0, pageSize: 200 } }); // RGB-Bulli b-channel hash
  // ToDo: See edge-case in resolveGroupItemsByGroupId
  const { raw: rawImages2 } = useImages({ raw: { filters: { tags: ['testing', '__filter:favourite__'] }, sort: { field: 'created', direction: 'asc' }, page: 0, pageSize: 200 } }); // RGB-Bulli b-channel hash
