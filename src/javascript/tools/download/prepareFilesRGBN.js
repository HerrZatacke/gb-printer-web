...
const prepareFilesRGBN = (exportScaleFactors) => (tiles) => {
  ...
  canvas.width = 160;
  const rgbnDecoder = new RGBNDecoder();
  rgbnDecoder.update(canvas, RGBNDecoder.rgbnTiles(tiles));
  ...
  Promise.all(images).then(download('rgbn'));
};

export default prepareFilesRGBN;
