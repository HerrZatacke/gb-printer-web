...
const getTransformBitmap = (store) => (file, fromPrinter = false) => {
  ...
  img.onload = () => {
    ...
    if (ratio === 112 / 128) {
      ...
      scaleFactor = img.naturalWidth / 160;
    }

    // if an image has the ratio of the "sensor" resolution of 128x112, import it inside a black frame
    if (ratio === 112 / 128) {
      context.drawImage(img, 0, 0, img.naturalWidth, img.naturalHeight, 16, 16, canvas.width - 32, canvas.height - 32);
    } else {
    ...
    store.dispatch({
      ...
      payload: {
        ...
        questions: getQuestions({ frameIds, frameGroups, fileName: file.name, scaleFactor }),
